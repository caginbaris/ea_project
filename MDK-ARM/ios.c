...
void inputHandling(){
	...
	input.bit.offDelayed						=off_delay(input.bit.raw,input.bit.onDelayed,holdTime,&counter[1]);
	input.bit.on_offDelayed					=on_off_delay(input.bit.raw,input.bit.onDelayed,holdTime,&counter[2]);
